...
export const appRouter = router({
  ...
  ai: router({
    recommend: publicProcedure.input(z.object({ caseId: z.string(), customer: z.string(), amount: z.number().finite().positive().max(1_000_000), failureReason: z.enum(["network_error", "bank_unavailable", "insufficient_funds", "expired_card", "invalid_payment_method", "suspected_fraud", "unknown_error"]), retryCount: z.number().int().min(0).max(3), consent: z.boolean(), fraudFlag: z.boolean(), daysSinceFailure: z.number().int().min(0).max(3650) })).mutation(async ({ input }) => {
      const response = await invokeLLM({
        messages: [
          { role: "system", content: "You are MendSync's clinical patient care diagnosis engine. Recommend only a bounded simulated action. Respect patient consent, fraud rules, and cooling limits. Return only the requested structured object." },
    ...
    recommendBatch: publicProcedure.input(z.object({ cases: z.array(z.object({ caseId: z.string(), customer: z.string(), amount: z.number(), failureReason: z.enum(["network_error", "bank_unavailable", "insufficient_funds", "expired_card", "invalid_payment_method", "suspected_fraud", "unknown_error"]), retryCount: z.number().int().min(0).max(3), consent: z.boolean(), fraudFlag: z.boolean(), daysSinceFailure: z.number().int().min(0).max(3650) })).max(60) })).mutation(async ({ input }) => {
      const response = await invokeLLM({
        messages: [
          { role: "system", content: "You are MendSync's batch patient care diagnosis engine. For every case, recommend only one bounded simulated action. Respect patient consent, fraud rules, and cooling limits. Return one decision per case in the same order. Return only the requested structured JSON array." },
